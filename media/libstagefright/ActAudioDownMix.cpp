...
downmix_int16 downmix_bufinu[DOWNMIX_NCHANS];// The first output channel for downmix

//downmix_int32 downmix_buf[DOWNMIX_NCHANS][DOWNMIX_N]; // Start domix after saving data of buf,after,all these channels need to move

downmix_int32 downmix_tab[DOWNMIX_NPCMCHANS][DOWNMIX_NPCMCHANS]; //domnmix table of channels
...
downmix_int16 downmix_chantab[DOWNMIX_NACMOD][DOWNMIX_NPCMCHANS] = {
    ...
    /*{
        DOWNMIX_CNTR, DOWNMIX_LEFT, DOWNMIX_RGHT, DOWNMIX_LSUR, DOWNMIX_RSUR, DOWNMIX_LFE
    }       /* AAC 5.1 3/2 */
};
...
downmix_int32 downmix_set(downmix_state *downmix_ac3_str,
    ...
    downmix_ac3_str->acmod = DOWNMIX_MODE32ACT; // Input coding mode 0-8
    downmix_ac3_str->outmod = DOWNMIX_MODE20; // Export PCM mode to 0-8
    downmix_ac3_str->cmixlev = 1; // certer channel level
    downmix_ac3_str->surmixlev = 1; //Sur channel level
    downmix_ac3_str->lfeon = 1; // Subwoofer channel level
    downmix_ac3_str->outlfe = 0; // Bass channel level
    downmix_ac3_str->dualmod = 0; // Dual-channel output mode, dual mono downmix mode 4 modes
    ...
    outDOWNMIX_NCHANS = (downmix_int16)(downmix_chanary[outmod] + outlfe); // Determined number of output channels;
...
downmix_int32 downmix_run(downmix_state *dommix_str, downmix_int32 *tcbuf,
        downmix_int32 frame_len, downmix_int32 channum) // input channel level
{
